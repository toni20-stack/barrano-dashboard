...
export function calcCostTotal(produs) {
  const base = produs.costAchizitieNIR !== undefined && produs.costAchizitieNIR !== ''
    ? (Number(produs.costAchizitieNIR) || 0)
    : (Number(produs.costAchizitie) || 0) + (Number(produs.transport) || 0) + (Number(produs.taxeVamale) || 0)
  const ambalaj = Number(produs.ambalaj) || 0
  const custom = (produs.componente || []).reduce((sum, c) => sum + (Number(c.suma) || 0), 0)
  return base + ambalaj + custom
}

export function calcVanzareProfit(vanzare, produse, opts = {}) {
  ...
  let comision = 0
  if (vanzare.canal === 'emag') {
    if (opts.applyProductComision && Number(produs.comisionEmag) > 0) {
      comision = venit * (Number(produs.comisionEmag) / 100)
    } else if (vanzare.aplicaComision) {
      comision = venit * (Number(vanzare.comisionEmag) || 0) / 100
    }
  }
