...
export function initStorage() {
  ...
  // Migrare categorii cheltuieli: Logistică → Transport, Salarii → Altele
  const ch2 = JSON.parse(localStorage.getItem(KEYS.cheltuieli) || '[]')
  const migratedCh2 = ch2.map(c => {
    if (c.categorie === 'Logistică') return {...c, categorie: 'Transport'}
    if (c.categorie === 'Salarii') return {...c, categorie: 'Altele'}
    return c
  })
