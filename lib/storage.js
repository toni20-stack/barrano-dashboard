...
export function initStorage() {
  ...
  const migratedCh = ch.map(c => (c.sursa === 'emag_ads' && !c.tara) ? {...c, tara: 'RO'} : c)
  if (migratedCh.some((c, i) => c !== ch[i])) {
    localStorage.setItem(KEYS.cheltuieli, JSON.stringify(migratedCh))
  }
  // Migrare: vânzări cu canal='altele' → canal='emag' (stornourile SmartBill)
  const vz = JSON.parse(localStorage.getItem(KEYS.vanzari) || '[]')
  const migratedVz = vz.map(v => v.canal === 'altele' ? {...v, canal: 'emag'} : v)
  if (migratedVz.some((v, i) => v !== vz[i])) {
    localStorage.setItem(KEYS.vanzari, JSON.stringify(migratedVz))
  }
  // Migrare categorii cheltuieli
  const ch2 = JSON.parse(localStorage.getItem(KEYS.cheltuieli) || '[]')
  const migratedCh2 = ch2.map(c => {
    const cat = c.categorie || ''
    if (cat === 'Logistică') return {...c, categorie: 'Transport'}
    if (cat === 'Salarii') return {...c, categorie: 'Altele'}
    if (cat === 'Card cadou retur eMAG') return {...c, categorie: 'Altele'}
    if (cat.startsWith('Comision Genius eMAG')) return {...c, categorie: 'Abonamente'}
    if (cat.startsWith('Comision transport cross-border eMAG')) return {...c, categorie: 'Transport'}
    if (cat.startsWith('Handling fee eMAG')) return {...c, categorie: 'Comisioane eMAG'}
    if (cat.startsWith('Comisioane eMAG')) return {...c, categorie: 'Comisioane eMAG'}
    return c
  })
  if (migratedCh2.some((c, i) => c !== ch2[i])) {
    localStorage.setItem(KEYS.cheltuieli, JSON.stringify(migratedCh2))
  }
}
